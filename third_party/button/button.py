...
class Button(object):
    ...
    def on_click(self, event):
        if self.rect.collidepoint(Button.application.shrink_scaled_pos(event.pos)):
            self.clicked = True
    ...
    def check_hover(self):
        if self.rect.collidepoint(Button.application.shrink_scaled_pos(pg.mouse.get_pos())):
            if not self.hovered:
                self.hovered = True
                if self.hover_sound:
